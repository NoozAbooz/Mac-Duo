...
/// perspective, then takes one hardware-filtered sample from the Gaussian
/// pyramid at the level chosen by the blur wanted there.
enum DepthShaders {
    static let source = """
    ...
    fragment float4 depthFragment(float4 position [[position]],
        ...
        float outputScale = uniforms.shape.z;
        ...
        // with y up.
        float2 screenPoint = float2(position.x / outputScale,
                                    screenSize.y - position.y / outputScale);
